...
export default class {
  ...
  JsCallApp(func, params = {}, callback = null) {
    ...
    try {
      if (this.isIOS) {
        window.__WebBridge.handleMessageFromJS(data)
      } else {
  ...
  appCallJsCallback(callbackId, params) {
    const data = JSON.stringify({ callbackId, params })
    if (this.isIOS) {
      return window.__WebBridge.handleCallbackFromJS(data)
    }
    return window.__AndroidBridge.handleCallbackFromJS(data)
  }
}
