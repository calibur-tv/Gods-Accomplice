...
const addRoute = (appName) => {
  ...
  // 模板对应的mock数据
  const tplDataMap = fs.readdirSync(dataPath)
    .filter(_ => _[0] !== '.') // 过滤以 . 开头的文件
    .reduce((result, file) => {
    const filePath = path.resolve(dataPath, file);
    const fileName = path.basename(file, '.json').replace(new RegExp(`^${appName}-`), '');
    result[fileName] = {
  ...
  Object.keys(tplDataMap).forEach((version) => {
    ...
    router.get(`/${version}`, (req, res) => {
      res.render(`${appName}/${majorVersion}/${majorVersion}`, tplDataMap[version]);
    });
  });
